...
impl Model for ThreeDof {
    ...
    fn step(&mut self, input: Self::Input, delta_t: f64) -> Self::State {
        ...
        self.state.inertial_frame_angle_velocity.z +=
            delta_t * self.state.inertial_frame_angle_acceleration.z;

        // Velocities (Inertial frame)
        self.state.inertial_frame_velocity.x =
            AIRSPEED_HORIZONTAL * f64::cos(self.state.inertial_frame_angle.z) + self.wind.x;
        self.state.inertial_frame_velocity.y =
            AIRSPEED_HORIZONTAL * f64::sin(self.state.inertial_frame_angle.z) + self.wind.y;
        ...
        self.state.body_frame_angle_acceleration =
            rotation * self.state.inertial_frame_angle_acceleration;
